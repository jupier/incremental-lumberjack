...
export function hitTreeAtTile(
  ...
  if (tile && tile.item === "tree" && tile.tree) {
    ...
    // Check if tree is already being destroyed (marked as destroyed)
    if ((targetTree as any).isDestroyed || (targetTree as any).health <= 0) {
      return; // Tree is already destroyed, don't hit it again
    }
    ...
    shakeTree(targetTree);
      ...
      if (newHealth <= 0) {
        // Mark tree as destroyed immediately to prevent multiple hits during animation
        (targetTree as any).isDestroyed = true;
        (targetTree as any).health = 0;
        
        // Update tile data immediately to prevent further hits
        tile.item = null;
        // Keep tile.tree reference for cleanup, but mark it as destroyed
